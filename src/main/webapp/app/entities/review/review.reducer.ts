...
export const ACTION_TYPES = {
  SEARCH_REVIEWS: 'review/SEARCH_REVIEWS',
...
export default (state: ReviewState = initialState, action): ReviewState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_REVIEWS):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_REVIEWS):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_REVIEWS):
...
const apiSearchUrl = 'api/_search/reviews';

// Actions

export const getSearchEntities: ICrudSearchAction<IReview> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_REVIEWS,
  payload: axios.get<IReview>(`${apiSearchUrl}?query=${query}${sort ? `&page=${page}&size=${size}&sort=${sort}` : ''}`),
});
