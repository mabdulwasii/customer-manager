...
import ng.com.dpros.customermanager.config.KafkaProperties;
...
class CustomerManagerKafkaResourceIT {
    ...
    void setup() {
        ...
        CustomerManagerKafkaResource kafkaResource = new CustomerManagerKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/customer-manager-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/customer-manager-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
